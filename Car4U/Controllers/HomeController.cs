...
namespace Car4U.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index([Bind(Include = "BeginDate,BeginHour,EndDate,EndHour,CategoryID,MPDeliveryID,MPReturnID")] InfoSender info)
        {
            ...
            return RedirectToAction("Create", "Reservations", new { mpreliveryid = info.MPDeliveryID, mpreturnid = info.MPReturnID, categotyid = info.CategoryID, begindate = info.BeginDate.ToString("yyyy-MM-dd"), beginhour = info.BeginHour.ToString("HH:mm"), enddate = info.EndDate.ToString("yyyy-MM-dd"), endhour = info.EndHour.ToString("HH:mm") });
        }
